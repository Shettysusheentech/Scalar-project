...
from .tasks import TASKS, make_reward
...
class NexusSocialEnv:
    ...
    def _finish(self, reward: Reward, info: Dict[str, Any] | None = None) -> Tuple[Observation, Reward, bool, Dict[str, Any]]:
        self.done = True
        return self.current_observation, reward, True, self._with_grader_score(reward, info)

    def _with_grader_score(self, reward: Reward, info: Dict[str, Any] | None = None) -> Dict[str, Any]:
        payload = dict(info or {})
        payload.setdefault("grader_score", float(reward.score))
        payload.setdefault("last_action_error", None)
        return payload
    ...
    def step(self, action: Action) -> Tuple[Observation, Reward, bool, Dict[str, Any]]:
        ...
        if self.task_id == "hard_context_request" and self.current_step == 1:
            if action.action == ActionType.REQUEST_CONTEXT:
                reward = make_reward(
                    0.49,
                    "Correct first move: more context is needed before taking action.",
                )
                ...
                return self.current_observation, reward, False, self._with_grader_score(reward, {"message": "Additional context provided."})
            if action.action == ActionType.FLAG:
                reward = make_reward(
                    0.2,
                    "Escalation is cautious, but the preferred first move was to request context.",
                )
        ...
        if self.task_id == "hard_context_request" and self.current_step == 2:
            if action.action == ActionType.APPROVE and action.category == CategoryType.SAFE:
                reward = make_reward(
                    0.49,
                    "Correctly cleared the post after the benign gaming context was revealed.",
                )
            elif action.action == ActionType.FLAG:
                reward = make_reward(
                    0.2,
                    "Still too cautious after the follow-up context clarified the post.",
                )
            else:
                reward = make_reward(
                    0.01,
                    "The follow-up action did not match the clarified benign context.",
                )
            return self._finish(reward)
        ...
        if self.task_id == "hard_coordinated_behavior" and self.current_step == 1:
            if action.action == ActionType.FLAG:
                reward = make_reward(
                    0.39,
                    "Good first move: escalate the post while gathering evidence of coordination.",
                )
                self.current_observation = Observation(
                ...
                return self.current_observation, reward, False, self._with_grader_score(reward, {"message": "Investigation results provided."})
            if action.action == ActionType.REJECT and action.category == CategoryType.OTHER:
                reward = make_reward(
                    0.7,
                    "The final action is right, but the task expected an investigation step first.",
                )
                return self._finish(reward)
        ...
        if self.task_id == "hard_coordinated_behavior" and self.current_step == 2:
            if action.action == ActionType.REJECT and action.category == CategoryType.OTHER:
                reward = make_reward(
                    0.59,
                    "Correctly rejected the content after the investigation confirmed coordinated behavior.",
                )
            else:
                reward = make_reward(
                    0.01,
                    "The follow-up action did not address the confirmed coordinated behavior.",
                )
            return self._finish(reward)
