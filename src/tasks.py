...
def clip_score(score: float) -> float:
    return float(max(MIN_SCORE, min(MAX_SCORE, float(score))))


def make_reward(score: float, explanation: str) -> Reward:
    return Reward(score=clip_score(score), explanation=explanation)
...
class SpamGrader(Grader):
    def grade(self, action: Action, ground_truth: Dict[str, Any]) -> Reward:
        if action.action == ActionType.REJECT and action.category == CategoryType.SPAM:
            return make_reward(MAX_SCORE, "Correctly identified and rejected spam.")
        elif action.action == ActionType.REJECT:
            return make_reward(0.5, "Correctly rejected, but category was wrong.")
        elif action.category == CategoryType.SPAM:
            return make_reward(0.3, "Correctly identified spam, but took the wrong action.")
        return make_reward(MIN_SCORE, "Failed to identify spam.")
...
class PolicyGrader(Grader):
    def grade(self, action: Action, ground_truth: Dict[str, Any]) -> Reward:
        ...
        return make_reward(score, " ".join(explanation))

class ContextGrader(Grader):
    def grade(self, action: Action, ground_truth: Dict[str, Any]) -> Reward:
        if action.action == ActionType.REQUEST_CONTEXT:
            return make_reward(MAX_SCORE, "Correctly identified that more context was needed.")
        elif action.action == ActionType.FLAG:
            return make_reward(0.4, "Flagging is a safe fallback, but requesting context is better.")
        return make_reward(MIN_SCORE, "Premature decision without sufficient context.")


class CoordinatedBehaviorGrader(Grader):
    def grade(self, action: Action, ground_truth: Dict[str, Any]) -> Reward:
        if action.action == ActionType.REJECT and action.category == CategoryType.OTHER:
            return make_reward(MAX_SCORE, "Correctly identified coordinated inauthentic behavior.")
        if action.action == ActionType.FLAG:
            return make_reward(0.5, "Reasonable escalation, but confirmed CIB should be rejected.")
        if action.category == CategoryType.OTHER:
            return make_reward(0.2, "Category is plausible, but the action is too weak.")
        return make_reward(MIN_SCORE, "Failed to identify coordinated inauthentic behavior.")

# Task definitions
TASKS = {
    "easy_spam_detection": {
        "name": "Easy: Spam Detection",
        "description": "Identify and reject obvious spam or scam posts.",
        "difficulty": "easy",
    ...
    "medium_policy_nuance": {
        ...
        "description": "Distinguish between general complaints and targeted hate speech.",
        "difficulty": "medium",
    ...
    "hard_context_request": {
        ...
        "description": "Request more context before taking a final action on ambiguous threats.",
        "difficulty": "hard",
    ...
    "medium_misinformation": {
        ...
        "description": "Flag misinformation that can mislead users or cause harm.",
        "difficulty": "medium",
    ...
    "hard_coordinated_behavior": {
        ...
        "description": "Investigate and act on coordinated inauthentic behavior.",
        "difficulty": "hard",
        ...
        "grader": CoordinatedBehaviorGrader(),
        "max_steps": 2
    }
}
